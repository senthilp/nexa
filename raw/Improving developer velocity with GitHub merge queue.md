...
source: https://humanwhocodes.com/blog/2026/04/improving-developer-velocity-github-merge-queue/
author:
  - "[[Nicholas C. Zakas]]"
  - "[[Amazon]]"
published: Mon
created: 2026-04-08
description: GitHub merge queue reduces the manual churn of keeping pull requests current by automatically retesting them in order before merge.
tags:
---
...
The first step in setting up a GitHub merge queue is configuring CI for the queue. You can do that by adding the `merge_group` trigger [^1] to the same GitHub workflow file that runs CI for pull requests:
...
Because a merge queue takes all commits from the temporary branch and merges them into `HEAD`, it’s important to squash pull requests [^2] before they are merged. That ensures each pull request adds a single commit to `HEAD`, which makes individual pull requests much easier to revert if necessary. To do that:
